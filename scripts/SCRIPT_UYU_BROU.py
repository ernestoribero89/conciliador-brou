...
#   - Luego intenta:
#       a) suma total de candidatos del mismo día
#       b) subconjunto de candidatos del mismo día que cierre contra SAP
#   - No mezcla proveedores ni comisiones.
#   - Si hay varios bloques salariales el mismo día, puede elegir solo el subconjunto correcto.
...
def _subset_sum_indices(candidates, target, tolerancia):
    ...
    Implementación DP acotada:
    - trabaja en centésimos para evitar errores float
    - limita estados para no colgar Render
    """
    target_i = int(round(abs(target) * 100))
    tol_i = int(round(abs(tolerancia) * 100))

    # sumas posibles: suma -> lista de índices
    states = {0: []}
    max_states = 120000

    # ordenar de mayor a menor ayuda a encontrar rápido combinaciones grandes
    ...
    for bi in ordered:
        ...
        # protección para no disparar combinatoria en Render
        if len(states) > max_states:
            # conservar estados más cercanos al target
            closest = sorted(states.items(), key=lambda kv: abs(kv[0] - target_i))[:max_states // 2]
            states = dict(closest)

    # chequeo final por cercanía
...
for sr in sap:
    ...
    # 2) Match por suma/subconjunto, fecha exacta primero, luego ±3 días.
    for dias_offset in range(0, DIAS + 1):
        for sign in ([0] if dias_offset == 0 else [1, -1]):
            ...
            # 2a) Si todos los candidatos del día cierran, usar todos.
            total = sum(abs(bco[bi]['imp']) for bi in cands)
            if abs(total - imp_s) <= TOLERANCIA:
                chosen = cands
            else:
                # 2b) Si no, buscar subconjunto correcto.
